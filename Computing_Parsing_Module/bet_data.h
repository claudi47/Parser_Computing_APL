...
struct bet_data
{
	...
	string date, match, one, ics, two, gol, over, under;

	// constructor, the ":" is the initializer list
	// using move to prevent the second copy of the variable in the field
	bet_data(string date, string match, string one, string ics, string two, 
		string gol, string over, string under) : date(move(date)),
