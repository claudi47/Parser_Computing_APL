...
{
    // instantiation of a object Document with encoding UTF-8
    ...
    for (auto it = doc.Begin(); it != doc.End(); it++) {
        bet_data row((*it)["date"].GetString(), (*it)["match"].GetString(), (*it)["one"].GetString(), 
...
string bet_data::to_string() {
    return date + ", " + match + ", " + one + ", " + ics + ", " + two + ", " + gol + ", " + over + ", " + under;
}
